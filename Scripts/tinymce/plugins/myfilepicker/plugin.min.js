(function () {
    ...
    tinymce.create('tinymce.plugins.EXPEDIT.MyFilePicker', {
        ...
        */    
        init: function (ed, url) {
            ...
            //var eventz = ["AddUndo", "BeforeAddUndo", "BeforeExecCommand", "BeforeRenderUI", "BeforeSetContent", "ExecCommand", "GetContent", "LoadContent", "NodeChange", "ObjectResizeStart", "ObjectResized", "ObjectSelected", "PostProcess", "PreInit", "PreProcess", "ProgressState", "SaveContent", "SetAttrib", "activate", "blur", "change", "deactivate", "focus", "hide", "init", "redo", "remove", "reset", "show", "submit", "undo"];
            //eventz.forEach(function (i,d) {
            //    ed.on(i, function (e) {

            //        var tinyFrame = $(tinyMCE.activeEditor.contentDocument.firstElementChild);

            //        tinyFrame.find('.tiny').click(function () { alert(1) });
    ...
    window.cleanFunction.push(function (selected) {
        // ['tiny-img']
        debugger;
        selected.find('*').andSelf().filter('.tiny.tiny-img').empty();
        selected.find('*').andSelf().filter('.tiny-popover').remove();
    });


    window.cleanFunctions = function (selected) {
        window.cleanFunction.forEach(function (i, d) {
            i(selected);
        })
    }
        ...
        if (typeof e !== 'undefined' && typeof e.target !== 'undefined' && typeof e.target.editorContainer !== 'undefined' && e.target.editorContainer) {
            ...
            currentDocument.find('*').andSelf().filter('.tiny').each(function (i, selected) {
                ...
                $(selected).click(function (e) {
                    ...
                    if (!found) {
                        var html = '';
                        html += '<div id="pop-' + selected.id + '" class="tiny-popover fade bottom in" role="tooltip" style="bottom: 0px; left: 0px; top: initial; display: block;"><div class="arrow"></div>';
                        ...
                        $(e.currentTarget).closest('.tiny').after($(html)[0]);
                    }
                }
                );
