...
function findClose(str, opentag, closetag, count, start) {
...
InstaView.convert = function(wiki)
{
    ...
    function filter(str, tagname, opentag, closetag, tagcount) {
        ...
        for (var i = 0; i < tagcount; i++)
            opened += opentag;
        var r = new RegExp( opened + "\s*" + tagname, 'gi')
        var start = str.regexIndexOf(r, 0);
        while (start > -1) {
            var count = tagcount;
            var end = start + count;
            while (count > 0 && end != -1) {
                var nextClose = str.indexOf(closetag, end + 1);
                var nextOpen = str.indexOf(opentag, end + 1);
            ...
            start = str.regexIndexOf(r, start);
        }
        return str;
    }
