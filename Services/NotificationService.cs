...
namespace EXPEDIT.Share.Services
{
    ...
    public class NotificationService : INotificationService, IOrchardShellEvents
    {
        ...
        public bool RegisterDevice(string deviceType, string id, int? timezone)
        {
            ...
            if (isNew)
            {
                var jsJoined = string.Format("{{ \"TYP\" : \"R\", \"message\":\"Welcome to {0}.\", \"title\": \"You added a device to {0}.\", \"msgcnt\" : \"1\", \"ts\": \"{1}\" }}", _services.WorkContext.CurrentSite.SiteName, DateTime.UtcNow.ToString("o"));
        ...
        public bool Chat(string username, string message)
        {
            ...
            using (new TransactionScope(TransactionScopeOption.Suppress))
            {
                ...
                var js = string.Format("{{ \"TYP\" : \"C\", \"message\":\"{2}\", \"title\": \"{0} message from {1}.\", \"msgcnt\" : \"1\", \"ts\": \"{2}\" }}", _services.WorkContext.CurrentSite.SiteName, _services.WorkContext.CurrentUser.UserName, message, DateTime.UtcNow.ToString("o"));
